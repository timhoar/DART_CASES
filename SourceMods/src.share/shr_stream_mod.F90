...
#include "dynamic_vector_procdef.inc"
  ...
  subroutine shr_stream_init(strm,infoFile,yearFirst,yearLast,yearAlign,taxMode,rc)
    ...
    startTag =  "<dataSource>"
    endTag = "</dataSource>"
! KDR This might be the docn.streams file.
...
! KDR Allow changing the offset in the middle of a run
!     to fix the avhrr file offset error.
          write(s_logunit,F04) " WARNING: offset disagrees: stream, user, .bin file = ",k,strm(k)%offset,inpi
