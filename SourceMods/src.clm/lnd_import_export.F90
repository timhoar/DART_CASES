...
! /glade/work/raeder/Models/cesm2_1_relsd_m5.6/components/clm/src/cpl/lnd_import_export.F90
...
! KDR debug
          write(iulog,'(A,2I6,1pe14.3)') &
                'WARNING: Longwave down sent from the atmosphere model is <= 0.0. g,i,lwrad = ', &
                g,i, atm2lnd_inst%forc_lwrad_not_downscaled_grc(g)
          ! Fix it, instead of debugging, for now.
          atm2lnd_inst%forc_lwrad_not_downscaled_grc(g) = 1.0_r8
!           call endrun( sub//errstring )
! KDR end
       end if
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!  AMF 11/30/17 Remove ENDRUN for negative values, and replace with 1.0_r8 
!  TJH  1/31/18 Some of the preprocessed fluxes have numerical artifacts that leave them slightly negative
       if (atm2lnd_inst%forc_solad_grc(g,1) < 0.0_r8)  then
          write(iulog,*) 'WARNING: Indirect solar =', atm2lnd_inst%forc_solad_grc(g,1),' gridcell index = ', g           
          atm2lnd_inst%forc_solad_grc(g,1) = 1.0_r8
       end if
       if (atm2lnd_inst%forc_solad_grc(g,2) < 0.0_r8)  then
          write(iulog,*) 'WARNING: Diffuse solar =', atm2lnd_inst%forc_solad_grc(g,2),' gridcell index = ', g         
          atm2lnd_inst%forc_solad_grc(g,2) = 1.0_r8
       end if

       if (atm2lnd_inst%forc_solai_grc(g,1) < 0.0_r8) then
          write(iulog,*) 'WARNING: Indirect, vis solar =', atm2lnd_inst%forc_solai_grc(g,1),' gridcell index = ', g 
          atm2lnd_inst%forc_solai_grc(g,1) = 1.0_r8
       end if
       if (atm2lnd_inst%forc_solai_grc(g,2) < 0.0_r8)  then
          write(iulog,*) 'WARNING: Indirect, near IR solar =', atm2lnd_inst%forc_solai_grc(g,2),' gridcell index = ', g    
          atm2lnd_inst%forc_solai_grc(g,2) = 1.0_r8
       end if
       if (atm2lnd_inst%forc_solad_grc(g,1) < 0.0_r8 ) then
          write(iulog,*) 'WARNING: Direct, vis solar =', atm2lnd_inst%forc_solad_grc(g,1),' gridcell index = ', g 
          atm2lnd_inst%forc_solad_grc(g,1) = 1.0_r8
       end if
       if (atm2lnd_inst%forc_solad_grc(g,2) < 0.0_r8 ) then
          write(iulog,*) 'WARNING: Direct, near IR solar =', atm2lnd_inst%forc_solad_grc(g,2),' gridcell index = ', g 
          atm2lnd_inst%forc_solad_grc(g,2) = 1.0_r8
       end if

       if ( atm2lnd_inst%forc_q_not_downscaled_grc(g) < 0.0_r8 )then
          call endrun( sub//' ERROR: Bottom layer specific humidity sent from the atmosphere model is less than zero' )
       end if
