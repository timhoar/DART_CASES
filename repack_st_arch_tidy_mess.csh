...
#       and remove the rpointer and .h0. files.
   set month = 11
   if ($month == 12) then
      echo "Fix this kluge and the the others, below"
      exit
   endif
...
if ($do_forcing == true) then
   ...
   touch cmds_template
   set i = 1
# Kluge2 only need to do members 1-72
...
# Kluge2 to resume where 2016-11 died; files have been moved to Previous,
#        but I still need init to point to those files.
   ...
   touch mycmdfile
   @ task = 0
# Kluge2; only need to do (some of) ha2x3h
...
# Kluge3; start with member 73, the first which is missing the 2016 yearly files,
#         both lnd and rof
      # set i = 1
      set i = 73
...
# Kluge to resume where 2016-10 died; only do this section for non-land components
#          if ($components[$m] != 'lnd') then
            ls ${CASE}.$models[$m]_${NINST}.*.${year}.nc >& /dev/null
            if ($status == 0) then
               mkdir -p Previous
               mv ${CASE}.$models[$m]_${NINST}.*.${year}.nc Previous 
# Kluge3; this new error checking needs to be off while fixing 2016-11 (members 73-80).
...
#          endif
      ...
      @ tasks = 0
      @ type = 0
# Kluge; h0 is already done for land
#       if ($components[$m] == 'lnd') @ type++
      while ($type < 10)
...
#             @ tasks = $tasks + 1
            ...
            # Not necessarily whole ensemble size.
            # @ tasks = $tasks + $ensemble_size
            @ tasks = $tasks + $comp_ens_size
         endif
         @ type++
      end
